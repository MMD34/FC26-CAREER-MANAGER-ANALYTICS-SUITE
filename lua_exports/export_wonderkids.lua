...
-- PT §7.4 / CSV_CONTRACTS §7.4 — PLAYERS_SNAPSHOT with row filter age<=MAX_AGE AND potential>=POTENTIAL_MIN.
-- Per SCHEMA_NOTES §15.1 (Option A): display_name via GetPlayerName; string-name columns empty.
-- Output: %USERPROFILE%\Desktop\WONDERKIDS_DD_MM_YYYY.csv
require 'imports/other/helpers'

assert(IsInCM(), "Script must be executed in career mode")

local MAX_AGE = 21
local POTENTIAL_MIN = 85
...
local file_path = string.format("%s\\WONDERKIDS_%s.csv", desktop_path, export_date)
...
-- Caches (same structure as export_players_snapshot.lua).
...
LOGGER:LogInfo("Exporting wonderkids...")
...
while rec > 0 do
    ...
    local age = compute_age(birthdate)
    local potential = players_table:GetRecordFieldValue(rec, "potential")

    if age ~= nil and age <= MAX_AGE and potential ~= nil and potential >= POTENTIAL_MIN then
...
io.close(file)
LOGGER:LogInfo(string.format("Done: %s (%d rows; age<=%d, potential>=%d)",
    file_path, row_count, MAX_AGE, POTENTIAL_MIN))
